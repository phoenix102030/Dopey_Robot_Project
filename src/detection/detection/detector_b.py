...
class Detector(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.out_channels = 5 + NUM_CATEGORIES#DEFAULT IS 5, 15 for classification ids
        ...
        self.out_cells_x = 20
        self.out_cells_y = 15
        self.img_height = 480.0  #DEFINE WITH YOUR OWN IMAGES RESO
        self.img_width = 640.0 #DEFINE WITH YOUR OWN IMAGES RESO 

        #NON MAXIMUM SUPRESSION (merge multiple bounding boses)
    ...
    def out_to_bbs(
        ...
        Args:
            out (torch.tensor):
                The output tensor encoding the predicted bounding boxes.
                Shape (N, 5, self.out_cells_y, self.out_cells_x).
        ...
        for o in out:
            ...
            for bb_index in bb_indices:
                ...
                img_category = torch.argmax(o[5:, bb_index[0], bb_index[1]])
    ...
    def anns_to_target_batch(self, anns: Tuple[dict]) -> Tuple[torch.Tensor]:
        ...
        for i, ann in enumerate(anns):
            for box,label in zip  (ann["boxes"], ann['labels']):
                ...
                one_hot = nn.functional.one_hot(torch.tensor([int(label)]), num_classes=NUM_CATEGORIES).squeeze()
                target[i,5: , y_ind, x_ind] = one_hot
