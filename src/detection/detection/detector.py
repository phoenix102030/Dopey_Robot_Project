...
class Detector(nn.Module):
    ...
    def out_to_bbs(
        ...
        for o in out:
            ...
            for bb_index in bb_indices:
                ...
                # img_category =  o[5:, bb_index[0], bb_index[1]]
                # if img_category.numel() == 0:
                # # Handle the empty case, e.g., skip or assign a default category
                # # For example, let's skip this bounding box
                #     continue
                # else:
                #     # Proceed with the non-empty case
                #     category_scalar = img_category.item()
    ...
    def anns_to_target_batch(self, anns: Tuple[dict]) -> Tuple[torch.Tensor]:
        ...
        for i, ann in enumerate(anns):
            for box in ann["boxes"]: #, label in zip (ann["boxes"], ann["labels"]):
                ...
                # one_hot = nn.functional.one_hot(torch.tensor([int(label)]), num_classes=5).squeeze()
