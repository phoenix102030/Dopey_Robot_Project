...
Detection NN B
    - trying to integrate classifer with old NN to ROS using dataset B
...
class CameraObjectDetector(Node):
    def __init__(self):
        ...
        self.NN_threshold = 0.92
        self.NMS_threshold = 0.4 # bigger = less boxes
        #
        ########################

        self.last_time_processed = time.time()
        self.process_interval = 0.5  # seconds between processing
        ...
        self.create_subscription(String, '/NN_status', self.start_callback, 5)
        

        self.bridge = CvBridge()
        self.device = torch.device("cuda") #if torch.cuda.is_available() else "cpu") ##use CUDA instead of CPU
        self.get_logger().info(f"CUDA? Device: {self.device}")
        ...
        self.processed_image_pub = self.create_publisher(Image, '/NN/NN_detections', 10)
        self.center_point_pub = self.create_publisher(PointStamped, '/NN/center_point', 10)
        self.detection_pub = self.create_publisher(String, '/NN/detection_status',10)
        ...
        self.publisher = self.create_publisher(PointStamped, '/NN/map_points', 10) #data points on map from NN
        self.marker_publisher = self.create_publisher(Marker, '/NN/map_marker', 10) #visualization of NN detection to rviz
    ...
    def image_callback(self, msg: Image):
        ...
        if current_time - self.last_time_processed >= self.process_interval:
        ...
        for i, centroid in enumerate(centroids):
            ...
            if depths[i] is not None and depths[i] > 0:
                ...
                self.put_on_map(x_m, y_m, depths[i], labels_str[i], msg.header.stamp)
    ...
    def convert_image(self, msg):
        ...
        image = image.to(self.device)  # Move image tensor to GPU
    ...
    def get_output(self, image):
        ...
        """
        with torch.no_grad():
            output = self.detector(image).to(self.device) #cpu() #GPU
            bbs = self.detector.out_to_bbs(output, self.NN_threshold)  # Adjust threshold as needed
            #print the bbs and shape

            bbs = nms_b(bbs, self.NMS_threshold)  # Adjust IoU threshold as needed
        return bbs
    ...
    def display_onoff_state(self, cv_image):
            #put text  self.ready_to_process in green ON and if false red OFF
        if self.ready_to_process:
            cv2.putText(cv_image, "ready_to_process ON", (400, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
            cv2.putText(cv_image, "ready_to_process ON", (400, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 1)
        else:
            cv2.putText(cv_image, "ready_to_process OFF", (400, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
            cv2.putText(cv_image, "ready_to_process OFF", (400, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1)
    ...
    def get_depth(self, centroid_x, centroid_y, radius=3, max_depth=2.0):
        """
        ...
        Args:
            ...
            max_depth (float): The maximum depth value to consider. Return None if object is too far (would be inaccurate).
        ...
        if self.cloud is None:
            self.get_logger().info("Error getting depth: Point cloud is None")
            return None
        ...
        for i in range(centroid_x - radius, centroid_x + radius):
            for j in range(centroid_y - radius, centroid_y + radius):
                ...
                try:
                    ...
                    self.get_logger().info(f"Error getting depth, continuing: {e}")
                    continue
        ...
        mean_depths = np.mean(depths)
        if mean_depths > max_depth: #if depth is more than 2m, ignore
            self.get_logger().info(f"NN found stable object but its too far for accurate reading: {mean_depths}. Ignoring it.")
            return None
    ...
    def put_on_map(self, x, y, z, label_str, timestamp):
        ...
        marker.text = label_str  # Add the label string as text
    ...
    def load_category_dict(self):
        try:
            ...
            json_path = os.path.join(os.getenv('HOME'), "/home/user/KTH_Robotics_Dopey/src/detection/annotations/training_b.json")
